package org.usfirst.frc.team126.robot.sensors;

import edu.wpi.first.wpilibj.I2C;
import edu.wpi.first.wpilibj.Notifier;
import edu.wpi.first.wpilibj.PIDSource;
import edu.wpi.first.wpilibj.PIDSourceType;
import org.usfirst.frc.team126.robot.util.BoundingBox;
import org.usfirst.frc.team126.robot.util.config.SensorsSettings;
...
public class PixyI2C implements PIDSource {
    ...
    public PixyI2C() {
        ...
        PIXY_UPDATE_NOTIFIER.startPeriodic(0.5);
    }
    ...
    public void read(int signature) { // The signature should be which number object in
        ...
        if (rawData.length < 32) {
            Log.print(0, "PIXYI2C", "Byte array is broken!");
            return;
        }
        ...
        for (int i = 0; i <= 16; i++) {
            ...
            if (syncWord == 0xaa55) { // Check is first 2 bytes equal a "sync word", which indicates the start of a packet of valid data
                ...
                Log.print(0, "PIXYI2C", "Got Sync Word from Camera!");
                ...
                if (checksum != sig + BOXES[sig - 1].getX() + BOXES[sig - 1].getY() + BOXES[sig - 1].getWidth() + BOXES[sig - 1].getHeight()) {
                    BOXES[sig - 1] = null;
                    Log.print(0, "PIXYI2C", "Pixy Error! Invalid Data!");
                } else {
                    Log.print(0, "PIXYI2C", "Got Box Data " + sig-1);
                }
                break;
            }
        }
